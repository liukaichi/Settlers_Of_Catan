 ...
 */
@SuppressWarnings("serial") public class MapComponent extends JComponent
{

    public static final int HEX_IMAGE_WIDTH = 298;
    public static final int HEX_IMAGE_HEIGHT = 298; // 258;

    private static final int WORLD_WIDTH = HEX_IMAGE_WIDTH * 5 + HEX_IMAGE_WIDTH / 2;
    ...
    static
    {
        ...
        SETTLEMENT.add(new Point2D.Double(SETTLEMENT_WIDTH / 2, -SETTLEMENT_WALL_HEIGHT / 2));
        SETTLEMENT.add(new Point2D.Double(SETTLEMENT_WIDTH / 2, SETTLEMENT_WALL_HEIGHT / 2));
        SETTLEMENT.add(new Point2D.Double(-SETTLEMENT_WIDTH / 2, SETTLEMENT_WALL_HEIGHT / 2));
        SETTLEMENT.add(new Point2D.Double(-SETTLEMENT_WIDTH / 2, -SETTLEMENT_WALL_HEIGHT / 2));
        Point2D sMidBase = average(SETTLEMENT.get(1), SETTLEMENT.get(2));
        Point2D sHighPoint = new Point2D.Double(sMidBase.getX(), sMidBase.getY() - SETTLEMENT_ROOF_HEIGHT);
        ...
        Point2D cHighPoint = new Point2D.Double(cThreeQuarterBase.getX(), cThreeQuarterBase.getY() - CITY_ROOF_HEIGHT);
        CITY.add(cHighPoint);
        ...
        EDGE_POINTS.put(EdgeDirection.NorthWest,
                average(VERTEX_POINTS.get(VertexDirection.West), VERTEX_POINTS.get(VertexDirection.NorthWest)));
        EDGE_POINTS.put(EdgeDirection.North,
                average(VERTEX_POINTS.get(VertexDirection.NorthWest), VERTEX_POINTS.get(VertexDirection.NorthEast)));
        EDGE_POINTS.put(EdgeDirection.NorthEast,
                average(VERTEX_POINTS.get(VertexDirection.NorthEast), VERTEX_POINTS.get(VertexDirection.East)));
        EDGE_POINTS.put(EdgeDirection.SouthEast,
                average(VERTEX_POINTS.get(VertexDirection.East), VERTEX_POINTS.get(VertexDirection.SouthEast)));
        EDGE_POINTS.put(EdgeDirection.South,
                average(VERTEX_POINTS.get(VertexDirection.SouthEast), VERTEX_POINTS.get(VertexDirection.SouthWest)));
        EDGE_POINTS.put(EdgeDirection.SouthWest,
                average(VERTEX_POINTS.get(VertexDirection.SouthWest), VERTEX_POINTS.get(VertexDirection.West)));
    ...
    private void drawHexes(Graphics2D g2)
    {
        ...
        for (Map.Entry<HexLocation, HexType> entry : hexes.entrySet())
        {
            ...
            g2.drawImage(hexImage, (int) hexCorner.getX(), (int) hexCorner.getY(),
                    (int) (hexCorner.getX() + HEX_IMAGE_WIDTH), (int) (hexCorner.getY() + HEX_IMAGE_HEIGHT), 0, 0,
